 ...
 * "version" and "attribution". If the "id" attribute is not provided an identifier will be generated. The rest of
 * the optional attributes are left null if not specified.
 * 
...
public class DeployedComponentImporter implements ObjectImporter {
    ...
    public void setRegistry(HeterogeneousRegistry registry) {
        this.registry = registry = registry == null ? new NullHeterogeneousRegistry() : registry;
    }
     ...
     */
    private void saveOrUpdate(DeployedComponent component) {
        DeployedComponent existingComponent = findExistingComponent(component);
     ...
     */
    private DeployedComponent findExistingComponent(DeployedComponent newComponent) {
        String name = newComponent.getName();
        return componentDao.findUniqueInstanceByName(name);
    }
