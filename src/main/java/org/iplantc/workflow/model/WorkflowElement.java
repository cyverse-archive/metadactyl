...
/**
 * Represents a single generic element in a workflow. This abstract class contains all of the properties that
 * are common to all elements in a workflow.
 * 
...
public abstract class WorkflowElement implements RepresentableAsJson, NamedAndUnique {
     ...
     * identifier is not unique among all workflow elements, but only among all workflow elements of the same kind
     * For example, a template and a property type may share the same identifier, but two templates cannot share the
     * same identifier.
     */
    private long hid;
    ...
    /**
     * A brief description of the workflow element. This should be slightly more descriptive than the label so
     * that it can be used as tooltip text.
     */
